...
class EuclideanDistTracker:
    def __init__(self):
        ...
        #self.stop = 0
        self.et=0
        self.s1 = np.zeros((1,1000)) ##Start timer
        self.s2 = np.zeros((1,1000)) ##End Timer
        self.s = np.zeros((1,1000)) ##End - Start
        self.f = np.zeros(1000) ##Capture Flag
    ...
    def capture(self,img,x,y,h,w,sp,id):
        if(self.capf[id]==0):
            ...
            file = './TrafficRecord/' + n + '.jpg'
            cv2.imwrite(file, crop_img)
            self.count += 1
            filet = open("./SpeedRecord.txt", "a")
            if(sp>limit):
                file2 = './TrafficRecord/exceeded/' + n + '.jpg'
